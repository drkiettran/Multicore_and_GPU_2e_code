...
// printf("%i has %i\n", rank, particlesPerNode);
  ...
  assert (pic != 0 && p != 0 && changes != 0 && totalChanges != 0 && changesPerNode != 0);
  ...
  MPI_Get_address (p, &add1);
  MPI_Get_address (&(p[0].y), &add2);
  displ[0] = 0;
  displ[1] = add2 - add1;

  MPI_Type_create_struct (2, lengths, displ, types, &Point);
...
//        }
      if (numTotalChanges > 0)
        {
          MPI_Allgatherv (changes, numChanges, Point, totalChanges, changesPerNode, buffDispl, Point, MPI_COMM_WORLD);
...
//       }
  ...
  if (rank == 0)
    fprintf (stderr, "Remaining particles %i\n", particles);
