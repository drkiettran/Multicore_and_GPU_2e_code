/*
 ...
 To build use  : mpic++ -std=c++17 bucketsortRMA.cpp -o bucketsortRMA
 ============================================================================
 */
#include<mpi.h>
#include<stdlib.h>
...
const int MAX = 10000;
//*****************************************
void dump(unique_ptr<int[]> &d, int M)
{
  for (int i = 0; i < M; i++)
    cout << d[i] << " " ;
  cout << endl;
...
int main (int argc, char **argv)
{
  ...
  unique_ptr<int[]> data = make_unique<int[]>(M);
  ...
  MPI_Win_create (buckets.get(), N * maxItemsPerBucket * sizeof (int), sizeof (int), MPI_INFO_NULL, MPI_COMM_WORLD, &bucketWin);
  MPI_Win_create (inBucket.get(), N * sizeof (int), sizeof (int), MPI_INFO_NULL, MPI_COMM_WORLD, &cntWin);
  MPI_Win_create (data.get(), M * sizeof (int), sizeof (int), MPI_INFO_NULL, MPI_COMM_WORLD, &dataWin);
  ...
  for (int i = 0; i < N; i++)
    MPI_Get (&(data[recvOff[i]]), toRecv[i], MPI_INT, i, bucketOffset[rank], toRecv[i], MPI_INT, bucketWin);
