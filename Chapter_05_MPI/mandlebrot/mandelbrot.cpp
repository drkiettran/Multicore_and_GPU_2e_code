...
void MandelCompute::init (WorkItem * wi)
{
  ...
  if (img == nullptr || pixelsX != wi->pixelsX || pixelsY != wi->pixelsY)
    {
      img = make_unique<int[]>((wi->pixelsX) * (wi->pixelsY));
    }
  pixelsX = wi->pixelsX;
  pixelsY = wi->pixelsY;
}
  ...
  if (rank == 0)   // master code
    {
      ...
      unique_ptr<WorkItem[]> w = make_unique<WorkItem[]>(Xparts * Yparts);
      ...
      unique_ptr<int[]> assignedPart = make_unique<int[]>(N);   // keep track of what its worker is assigned
      unique_ptr<int[]> imgPart = make_unique<int[]>(workItemPixelsPerSide * workItemPixelsPerSide);
      for (int i = 0; i < Xparts * Yparts; i++)
        {
          MPI_Recv (imgPart.get(), workItemPixelsPerSide * workItemPixelsPerSide, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
          int workerID = status.MPI_SOURCE;
          int tag = status.MPI_TAG;
          int widx = assignedPart[workerID];
          assignedPart[workerID] = i;
          MPI_Issend (&(w[i]), 1, workItemType, workerID, WORKITEMTAG, MPI_COMM_WORLD, &request); 

          if (tag == RESULTTAG)
            {
              savePixels (img, imgPart.get(), w[widx].imageX, w[widx].imageY, workItemPixelsPerSide, workItemPixelsPerSide);
            }
        }
      ...
      while (1)
        {
          ...
          c.init (&w);
          c.compute ();
          MPI_Send (c.img.get(), w.pixelsX * w.pixelsY, MPI_INT, 0, RESULTTAG, MPI_COMM_WORLD); // return the results
        }
    }
