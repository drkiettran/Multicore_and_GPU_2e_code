/*
 ...
 Description   : 
 To build use  : nvcc data_manage_example.cu -o data_manage_example
 ============================================================================
 */
...
int main ()
{
  ...
  endsPair = thrust::unique_by_key_copy(h_keys.begin(), h_keys.end(), h_values.begin(), dest_keys.begin(), dest_values.begin());
  dest_keys.resize(endsPair.first - dest_keys.begin());
  dest_values.resize(endsPair.second - dest_values.begin());
  print("unique_by_key_copy (keys)  : ", dest_keys);
  print("unique_by_key_copy (values): ", dest_values);
  // Output is:
  // unique_by_key_copy (keys)  : 1 2 3 4 5 6 7 
  // unique_by_key_copy (values): B E C F A I H 
