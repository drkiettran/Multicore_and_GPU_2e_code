/*
 ...
 To build use  : g++ -fopenmp mergesort_omp_topdown_v2.cpp -o mergesort_omp_topdown_v2
 ============================================================================
 */
#include <stdlib.h>
#include <string.h>
...
#pragma omp task
      {
        mergesortRec (data, temp.get(), middle);
      }
#pragma omp task
      {
        mergesortRec (data + middle, temp.get() + middle, N - middle);
      }

#pragma omp taskwait

      mergeList (data, data + middle, middle, N - middle, temp.get());
    }
  }
}
...
int main (int argc, char *argv[])
{
  ...
  numberGen (N, MAXVALUE, data.get());

  double t = omp_get_wtime ();
  mergesort (data.get(), N);
