...
int main (int argc, char **argv)
{
  ...
  unique_ptr<float[]> pcCos = make_unique<float[]> (degreeBins);
  unique_ptr<float[]> pcSin = make_unique<float[]> (degreeBins);
  float rad = 0;
  ...
  cudaMemcpyToSymbol (d_Cos, pcCos.get(), sizeof (float) * degreeBins);
  cudaMemcpyToSymbol (d_Sin, pcSin.get(), sizeof (float) * degreeBins);

  // setup and copy data from host to device
  unsigned char *d_in, *h_in;
  int *d_hough;
  unique_ptr<int[]> h_hough;

  h_in = inImg.pixels; // h_in is just an alias here

  h_hough = make_unique<int[]>(degreeBins * rBins);
  ...
  cudaMemcpy (h_hough.get(), d_hough, sizeof (int) * degreeBins * rBins, cudaMemcpyDeviceToHost);
