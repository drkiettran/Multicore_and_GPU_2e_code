...
int main (int argc, char *argv[])
{
  ...
  char *d_data;
  unique_ptr<char[]> h_data;
  cudaMalloc ((void **) &d_data, MAX_MESG);
  h_data = make_unique<char[]>(MAX_MESG);
  ...
  if (rank == 0)
    {
          ...
          for (int i = 0; i < REP; i++)
            {
              ...
              cudaMemcpy (h_data.get(), d_data, mesg_size, cudaMemcpyDeviceToHost);
              // send it to other host
              MPI_Send (h_data.get(), mesg_size, MPI_CHAR, 1, tag, MPI_COMM_WORLD);
              // wait to collect response
              MPI_Recv (h_data.get(), MAX_MESG, MPI_CHAR, 1, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
	      // forward data to device
              cudaMemcpy (d_data, h_data.get(), mesg_size, cudaMemcpyHostToDevice);
            }
      ...
      tag = END_TAG;
      MPI_Send (h_data.get(), 0, MPI_CHAR, 1, tag, MPI_COMM_WORLD);
    }
  else
    {
      while (1)
        {
          // get message
          MPI_Recv (h_data.get(), MAX_MESG, MPI_CHAR, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
          // filter-out end tag
          ...
          cudaMemcpy (h_data.get(), d_data, mesg_size, cudaMemcpyDeviceToHost);
          // and send it back
          MPI_Send (h_data.get(), mesg_size, MPI_CHAR, 0, tag, MPI_COMM_WORLD);
        }
    }
