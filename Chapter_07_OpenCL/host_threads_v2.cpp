/*
 ...
 To build use  : icc -lOpenCL -lpthread host_threads_v2.cpp  -o host_threads_v2
 ============================================================================
 */
...
//============================================
void hostThr (int ID, cl_kernel * kern)
{
  int errNum;
...
int main (int argc, char **argv)
{
  ...
  cl_kernel kernCopies[N];
  for (int i = 0; i < N; i++)
    {
      kernCopies[i] = clCloneKernel (kern, &errNum);
      if (isError (errNum, "Failed to copy kernel"))
        {
          cleanUp ();
          return 1;
        }

      thr[i] = make_unique < thread > (hostThr, i, &(kernCopies[i]));
    }
